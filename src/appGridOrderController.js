...
export class AppGridOrderController {
    ...
    _saveCurrentManualOrder() {
        if (!this._isDragging) return;

        const items = this._appDisplay?._orderedItems;
    ...
    _refreshAppGrid() {
        ...
        console.log(
            items
                .map((item, index) => `${index}: ${item.name ?? item.id}`)
                .join(" | "),
        );
        ...
        const gridItems = grid.getItemsAtPage(0) ?? [];
        ...
        GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
            console.log("========== ALT GRID AFTER ALLOCATION ==========");

            const itemsAfter = grid.getItemsAtPage(0) ?? [];
